...
export const sitesService = {
  ...
  function getUTMParams() {
    // Try to read fresh UTM from URL first (landing page); fall back to session storage
    ...
    // Persist if any UTM present so subsequent pages retain them
    var hasUTM = utm.utm_source || utm.utm_medium || utm.utm_campaign;
    if (hasUTM) {
      sessionStorage.setItem('_analytics_utm', JSON.stringify(utm));
    } else {
      var stored = sessionStorage.getItem('_analytics_utm');
