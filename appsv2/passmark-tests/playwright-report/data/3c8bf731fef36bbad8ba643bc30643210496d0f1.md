...
- Name: dashboard/analytics-sections.spec.ts >> Engagement (/engagement) >> renders heading and engagement metrics
- Location: tests/dashboard/analytics-sections.spec.ts:189:3

# Error details

```
Error: The accessibility snapshot and current URL do not show any heading or text labeled 'Engagement'. Multiple headings related to analytics and features are present, but none mention 'Engagement' explicitly or implicitly. Therefore, the assertion that an 'Engagement' heading is visible does not hold based on the provided data.
