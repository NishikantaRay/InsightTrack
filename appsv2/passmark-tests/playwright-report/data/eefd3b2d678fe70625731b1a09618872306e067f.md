...
- Name: dashboard/analytics-sections.spec.ts >> Performance (/performance) >> renders heading and performance metrics
- Location: tests/dashboard/analytics-sections.spec.ts:135:3

# Error details

```
Error: The accessibility snapshot of the page at the URL http://localhost:4173/landing does not contain any heading or text labeled 'Performance'. Multiple headings related to features, system architecture, and other topics are present, but none mention 'Performance'. Therefore, the assertion that a 'Performance' heading is visible does not hold true based on the provided snapshot.
