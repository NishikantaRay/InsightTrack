...
- Name: dashboard/analytics-sections.spec.ts >> Conversions (/conversions) >> renders heading and conversion metrics
- Location: tests/dashboard/analytics-sections.spec.ts:27:3

# Error details

```
Error: The accessibility snapshot and current URL do not show any heading containing the words 'Conversions' or 'Conversions & Funnels'. There is a heading 'Conversion Funnels' but it does not exactly match the assertion text and the assertion specifically asks for 'Conversions' or 'Conversions & Funnels'. Given the rules to focus on the intent, 'Conversion Funnels' is related but not the exact term, so the assertion does not fully pass.
