...
- Name: dashboard/analytics-sections.spec.ts >> Acquisition (/acquisition) >> renders heading and traffic source breakdown
- Location: tests/dashboard/analytics-sections.spec.ts:108:3

# Error details

```
Error: The accessibility snapshot of the page at the URL http://localhost:4173/landing does not contain any heading with the text 'Acquisition' or a similar term. Multiple headings are present, but none match or imply 'Acquisition'. Therefore, the assertion that an 'Acquisition' heading is visible does not hold based on the provided snapshot.
