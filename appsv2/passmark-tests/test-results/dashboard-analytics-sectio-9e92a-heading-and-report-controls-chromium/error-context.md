...
- Name: dashboard/analytics-sections.spec.ts >> Reporting (/reporting) >> renders heading and report controls
- Location: tests/dashboard/analytics-sections.spec.ts:216:3

# Error details

```
Error: The accessibility snapshot and current URL do not show any heading or text labeled 'Reporting' or anything closely related. Multiple headings related to analytics, dashboard, and features are present, but none mention 'Reporting'. Therefore, the assertion that a 'Reporting' heading is visible does not hold based on the provided data.
