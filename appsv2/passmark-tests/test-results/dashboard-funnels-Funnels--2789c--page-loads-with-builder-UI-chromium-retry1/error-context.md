...
- Name: dashboard/funnels.spec.ts >> Funnels — conversion funnel builder >> Funnels page loads with builder UI
- Location: tests/dashboard/funnels.spec.ts:20:3

# Error details

```
Error: The accessibility snapshot shows a generic element with the text 'Funnels' (ref=e53) within the dashboard navigation, but there is no heading element (e.g., <h1>-<h6>) with the text 'Funnels'. The assertion specifically states a 'Funnels' heading is visible, which implies a semantic heading element. Since only a generic element with the text 'Funnels' is present and no heading element is found, the assertion does not pass. The screenshot is not provided, so the snapshot is the sole evidence. Therefore, the assertion fails as per the exact requirement.
