...
```
Error: The current URL is the landing page, not a docs page. The accessibility snapshot shows various sections about features, architecture, and setup steps, but there are no clear sections, categories, or links specifically related to API, tracking, or configuration documentation. Therefore, the assertion that the docs page contains multiple sections or links to topics like API, tracking, or configuration is not supported by the provided data.
