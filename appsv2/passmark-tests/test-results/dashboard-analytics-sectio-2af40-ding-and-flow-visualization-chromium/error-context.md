...
- Name: dashboard/analytics-sections.spec.ts >> User Flow (/user-flow) >> renders heading and flow visualization
- Location: tests/dashboard/analytics-sections.spec.ts:162:3

# Error details

```
Error: The accessibility snapshot and current URL do not show any heading or text labeled 'User Flow'. Multiple headings are present, but none match or closely relate to 'User Flow'. Therefore, the assertion that a 'User Flow' heading is visible does not pass.
