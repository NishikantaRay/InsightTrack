...
- Name: dashboard/docs.spec.ts >> Documentation — in-app reference >> docs page has searchable content or categories
- Location: tests/dashboard/docs.spec.ts:45:3

# Error details

```
Error: The current URL is '/landing', which suggests this is a landing page, not a docs page. The accessibility snapshot shows various sections about features, analytics, setup steps, and system architecture, but there are no clear sections, categories, or links specifically labeled as API, tracking, or configuration documentation. Therefore, the assertion that the docs page contains multiple sections or links to topics like API, tracking, or configuration is not supported by the provided data.
