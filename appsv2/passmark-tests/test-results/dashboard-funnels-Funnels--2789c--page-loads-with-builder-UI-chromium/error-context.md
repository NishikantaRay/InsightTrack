...
```
Error: The accessibility snapshot shows the text 'Funnels' as a generic element within the dashboard menu, but there is no heading element (e.g., <h1>, <h2>, <h3>, etc.) with the text 'Funnels'. The assertion specifically states a 'Funnels' heading is visible, which implies a semantic heading element. Since only a label or link with the text 'Funnels' is present and no heading element is found, the assertion does not pass.
