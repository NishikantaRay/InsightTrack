...
```typescript
// AI steps for navigation
await page.goto('/login');
await page.waitForSelector('input[placeholder="you@company.com"]');

// Raw Playwright for attribute checks
...
// Back to AI for visual assertion
...
```bash
# All 52 tests (takes ~40-50 min with gpt-4.1-mini)
npm test
...
# Show live output as each test runs
...
Passmark routes all AI calls through [OpenRouter](https://openrouter.ai), configured in `playwright.config.ts`. You can tune which model handles which part of the pipeline:
...
Check your balance before a full run: `curl https://openrouter.ai/api/v1/auth/key -H "Authorization: Bearer $OPENROUTER_API_KEY"`
...
→ OpenRouter routing to a `:free` suffix model that has been rate-limited. Make sure your `OPENROUTER_API_KEY` has credits and you're not accidentally using a free-tier key.
