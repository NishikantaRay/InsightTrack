import { createPool, query, closeConnection } from '../src/db/postgres.js';
...
async function aggregateDay(siteId, date) {
    ...
    const [eventsResult, sessionsResult, topPagesResult, sourcesResult, devicesResult, countriesResult] = await Promise.all([
        query(
            `SELECT COUNT(DISTINCT user_id) as visitors, COUNT(DISTINCT session_id) as sessions,
             COUNT(*) FILTER (WHERE type = 'pageview') as pageviews
             FROM events WHERE site_id = $1 AND timestamp >= $2 AND timestamp <= $3`,
            [siteId, dayStart, dayEnd]
        ),
        query(
            `SELECT COUNT(*) as total, COUNT(*) FILTER (WHERE is_bounce = TRUE) as bounces,
             AVG(duration) as avg_duration
             FROM sessions WHERE site_id = $1 AND started_at >= $2 AND started_at <= $3`,
            [siteId, dayStart, dayEnd]
        ),
        query(
        ...
        query(
            `SELECT CASE
               ...
               ELSE 'Referral'
             END as source, COUNT(DISTINCT user_id) as visitors
             FROM events WHERE site_id = $1 AND timestamp >= $2 AND timestamp <= $3
             GROUP BY source ORDER BY visitors DESC`,
            [siteId, dayStart, dayEnd]
        ),
        query(
            `SELECT CASE WHEN device = '' OR device IS NULL THEN 'Desktop' ELSE device END as device,
    ...
    await query(
        `INSERT INTO daily_stats (site_id, date, visitors, sessions, pageviews, bounces, avg_duration, top_pages, sources, devices, countries, computed_at)
         ...
         ON CONFLICT (site_id, date) DO UPDATE SET
           ...
           pageviews = EXCLUDED.pageviews, bounces = EXCLUDED.bounces,
           avg_duration = EXCLUDED.avg_duration, top_pages = EXCLUDED.top_pages,
           sources = EXCLUDED.sources, devices = EXCLUDED.devices,
           countries = EXCLUDED.countries, computed_at = NOW()`,
        [
            siteId, dateStr,
            Number(e.visitors || 0), Number(e.sessions || 0),
            Number(e.pageviews || 0), Number(s.bounces || 0),
